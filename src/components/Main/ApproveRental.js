...
const ApproveRentalContainer = styled.div`
  width: 100%;
  height: 100%;
  padding: 92px 56px;
...
const ApproveRentalTitle = styled.div`
  font-size: 24px;
  display: flex;
...
const ApproveRentalWrapper = styled.div`
  width: 100%;
  display: flex;
...
const ApproveRental = ({
  ...
  const clickRentalButton = () => {
    Swal.fire({
      title: "대여 신청 처리",
      icon: "warning ",
      text: "대여 신청을 처리하시겠습니까?",
      confirmButtonText: "확인",
      cancelButtonText: "취소",
      showCancelButton: true,
    }).then((res) => {
      if (res.isConfirmed) {
        approveRental(selectedRequest._id);
      } else {
        return;
      }
    });
  ...
  return (
    <ApproveRentalContainer>
      ...
      <ApproveRentalWrapper>
        ...
        <Button onClick={() => clickRentalButton()} size="Large" cancel={false}>
          대여 신청 처리
        </Button>
      </ApproveRentalWrapper>
    </ApproveRentalContainer>
